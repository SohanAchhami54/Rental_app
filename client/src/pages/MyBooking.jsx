            ...
            <li key={booking._id} className="grid grid-cols-1 md:grid-cols-4 gap-6 p-6 rounded-lg border border-gray-400 mt-5">
              ...
              <div className="flex flex-col mx-auto">
                ...
                {booking.status === 'pending' && booking.paymentAllowed&&(
                  <div className="mt-2">
                    <Button 
                    onClick={()=>navigate('/payment',{replace:true,state:{total_amount:booking.price,bookingId:booking._id}})}
                    className="bg-green-100 rounded-lg font-medium"
                    >
                    {
                      <>
                      <span className="text-xs md:text-base lg:text-md">Pay via</span> <img src={assets.eSewa} className="h-10 w-full" alt="" />
                      </>
